...
This project provides `/swarm` — a skill for coordinating parallel agents
over tmux. The skill definition is at `.claude/skills/swarm.md`.

When the user says "swarm", "coordinate agents", "ping-pong", "parallel
brainstorm", or wants multiple agents to collaborate, invoke the `/swarm`
skill.

The coordinator scripts live in `bin/swarm` and `lib/`. Dependencies:
tmux (3.2+), bash, jq, yq.
