...
- Tests must be run with bash, not zsh

- All buffer names must be scoped with SWARM_BUFFER_PREFIX

- Run markdownlint via podman, not local install — podman run --rm -v $PWD:/workdir:Z docker.io/davidanson/markdownlint-cli2:v0.22.0 is the canonical invocation. Use task lint:md as the entry point.
